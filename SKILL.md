...
- **First setup or another project:** read the [standard workflow](references/standard-workflow.md), then [bootstrap](references/bootstrap.md). For account/resource setup use [API onboarding](references/api-onboarding.md): official login, repository reconciliation, target checks and fixed TAT configuration. Use the bundled generator and fixed entry points; adapt project differences. The standard first-install path uses separate test and production hosts. Multiple projects do not imply a shared host.
...
- **Close out a completed release:** use [post-release closeout](references/post-release-closeout.md): `release-session.mjs verify` reads TAT/CNB execution evidence; `verify-coexistence.py` checks declared shared-host resources; `reconcile-project-state.py` updates the existing local status index. Historical execution can be verified after approval expiry; new signing or publishing still requires valid authorization. Resume from the document's managed environment block and private `environments.<env>.current`; report only the declared, verified acceptance scope.
