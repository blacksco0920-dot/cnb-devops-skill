...
后续[真实 API 验证](2026-09-07-api-live-validation.md)已发现默认 CNB CLI 登录缺少仓库创建权限，并确认腾讯云配置与复用可用。下文保留当时研究范围，实际使用以接入入口和后续验证为准。
...
同日按上述顺序落实首版，入口见[账号与 API 接入](../../references/api-onboarding.md)。新增 CNB 仓库/构建配置器、腾讯云官方登录适配与安全导出、CAM 专用用户/权限配置器；TAT 配置增加真实实例/Agent 就绪查询，运行客户端补齐 STS Token。运行包升为 `0.2.0-preview.2`，既有云端证据仍只对应记录内的原工件。

本机验证覆盖 CNB 创建/复用/类型冲突/未知结果续接，TAT 目标离线和绑定检查，CAM 身份权限漂移；另用真实固定版官方 CLI/模块配合隔离传输，验证参数、OAuth 刷新、腾讯云 loopback 回调、TLS 与错误脱敏。CI 纳入无需账号的腾讯云官方模块测试；未进行新的云登录、资源初始化或部署演练。

已发现并处理官方 tccli `3.1.162.1` 的 OAuth `verify=False` 与回调全网监听行为：进程内恢复证书校验，拒绝 HTTP 重定向，回调只绑定本机，错误返回脱敏，登录有界等待。仅安装官方 auth 模块所需依赖，不修改用户全局 CLI 或自建 OAuth 协议。

尚未自动化的边界：Secret 文件仍需官方 Web 保存；CAM 执行器只准备身份与权限，不签发长期密钥；OIDC 联邦、插件固定版本与三类真实流水线事件尚未做云端验收，不启用为默认。没有据此宣称全新账号接入耗时已达标。
