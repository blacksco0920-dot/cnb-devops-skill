...
- [x] Complete actual CRM test login/business/browser/recovery/update acceptance; the separately governed production result is recorded below.

## Execution record

2026-09-11: the approved repair flow is implemented at Skill commit `76595bb`. Controller upgrade, failed-source restore, fixed TAT update, complete source build and repair event passed. Accepted application is `8ddd8b73b45fd9e05d66229bc966253a4407f14c` / `cnb-qu8-1k26vakd2`; repair execution is `cnb-7ao-1k270ffpv`. Original failure/history/snapshots and the consumed permit remain independently verified. Real business and representative-data restore passed. Minimal mobile/startup fixes then passed ordinary update `c1210e8a` / `cnb-0ih-1k271p8iu`, real UI, shared-host checks, preserved data and another isolated restore. Separate production installation and approved same-image release `cnb-v4g-1k27bf08j` subsequently passed business, UI, coexistence and PostgreSQL/uploads isolated restore checks. Exact scope and timing are recorded in [shared-host validation](../../history/2026-09-09-shared-host-preparation.md).
