...
- 新生产入口：`node --test tests/test_release_session.mjs`，10 项通过。真实调用标准 Git gate、签名与授权验证、publisher；网络、TAT 和测试内依赖安装使用隔离替身。覆盖依赖漏装、下载错误、无产物签名重试、损坏签名保留、到期阻断、锁恢复及发布响应丢失后的回读续接。
