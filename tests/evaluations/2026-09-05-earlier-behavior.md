# Earlier behavioral evidence — archived 2026-09-05

Archived from `tests/skill-scenarios.md` at `a0b0772`. The source below preserves
reported observations and their original labels; “current”, “revised”, and
imperative evaluation instructions refer to that historical source, not the
present package. Known dates are retained; the legacy takeover evaluations have
no recorded date or exact tested commit. Other early records likewise do not
identify an exact tested commit or more specific model. Provenance remains
unverified beyond these reports. Use the [current catalog](../skill-scenarios.md)
for prompts and expected behavior, not this archive as an operational guide.

The Shared Caddy source says **12/12 PASS** but lists **13 PASS rows**. Both are
preserved unchanged below. The extra row cannot be assigned fresh-context
provenance from this record; no corrected total or thirteenth result is inferred.
Automated mappings establish test coverage only and do not validate those claims.
The historical takeover GREEN sequence also omitted rotation in its written
arrow list although its observation required a rotation receipt; the current
catalog has one explicit sequence including rotation.
...
## Historical legacy takeover evaluations
