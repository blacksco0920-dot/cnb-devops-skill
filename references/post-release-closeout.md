...
入口自动将本次 spec 的原路径和原字节摘要登记到 `current.closeout_spec`。新会话从项目状态索引找到它，核对摘要后即可运行同一 `--spec` 的只读预览，不必重新生成输入。旧版索引没有此字段时，只沿已登记的执行记录查找；无法定位则报告这一输入缺口，不全盘搜索私密目录。更新入口后补登记旧收尾时，保留旧事务，使用新 spec 和新事务目录。
