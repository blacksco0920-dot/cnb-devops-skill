...
当前交付范围：**测试发布和自动候选的可复用包**，以及主机安装、原生 Caddy 接入和固定 TAT 命令配置工具。FinAgent 三服务已完成一次云端测试发布和自动候选验收，固定版本、主机条件和边界见[执行包实测](../docs/history/2026-09-06-bundle-staging-rehearsal.md)；生产入口保持阻断，恢复执行器尚未交付。支持 Ubuntu 24.04、Docker Compose、PostgreSQL、同仓同提交的应用与发布程序；外部共享网络及 HTTPS 路由需先接通。不兼容时列出具体差异，沿用项目已验收的路径。
...
首次接入的 Ubuntu 24.04 测试主机可使用 `host/bootstrap-host.py`：填写 `bootstrap-spec.json`，固定 PostgreSQL 16 / Redis 7 的 TCR 摘要及盘点得到的 APT 版本，`generate_env` 声明在主机生成的随机值。该预设安装缺失的 Docker/Compose，建立项目独立网络、数据库/用户和可选 Redis，运行秘密留在主机。已有资源必须属于同一安装记录；不接管预装 Caddy。本入口已在一台保留预装服务的重置主机上实际执行，无预装服务的纯净镜像和第二项目接入尚未验证。
...
安装器校验依赖、完整工件、权限和数据库起点，再安装固定程序与显式空库记录；不会创建数据库、重装公共组件或接管已有应用。重复调用不会覆盖运行中的项目配置，内容漂移停止。迁移后的失败保留事务现场，不能自动改回旧镜像假装恢复。本轮已实际完成空库安装与后续测试发布；完整重复安装、中断续接及共享主机第二项目仍需各自验收。安装器没有版本升级入口；本轮程序修复使用单独审核的管理员升级脚本，不能把它当成通用升级能力。
...
此入口已在 Caddy 2.11.4 云主机完成接入，保留原默认站点并核对文件与运行配置；本轮三个测试域名的 HTTPS、可用性和发布身份检查通过。这只覆盖记录中的拓扑，配置成功仍需该项目的真实访问与业务验收。
