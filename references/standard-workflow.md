...
本版已将测试发布器、TAT 调用与候选逻辑提取到[可复用包](../assets/cnb-tcr-tat/bundle.json)，并提供配置校验、确定性生成和固定命令配置入口。兼容项目直接按[接入步骤](bootstrap.md)使用，不再依赖维护者本机 ecat 目录。新包目前只经过本地验证；全新主机、生产与恢复的云端验收仍需分别完成。
