...
首次接入和新增项目先读本页。目标是复用已验证的阶段和工件，只适配业务差异；日常构建、部署和 Tag 生成由流水线执行。接入说明维护于业务仓库的 `docs/DEPLOYMENT.md`，当前进度沿用项目已有的权威状态文档；没有时再创建 `docs/PROJECT_STATUS.md`。

## 默认适用范围

包内已提供文件生成、SSH 首装、固定 TAT、测试与候选、生产授权和隔离恢复入口。固定运行包要求项目使用 PostgreSQL 16，不支持无数据库项目或其他数据库；Redis 7 可选。默认两台独立 Ubuntu 24.04 / Linux amd64 主机，应用与发布程序位于同仓同提交；生产只在显式配置后启用。完整配置从[通用示例](../assets/cnb-tcr-tat/project.full.example.yml)适配；具体兼容条件见[接入说明](bootstrap.md)。
...
1. **先判断是否适用。** 从项目源码和现有配置确认运行方式、数据库和持久数据。明确不适用时，保留已有代码和流水线，在权威状态文档记录具体限制、已完成检查及下一步；没有状态文档时仅新增一份简短记录。此时不展开身份接口、镜像、候选和审批页改造，也不收集暂时用不上的云参数。不能通过增设无业务需要的数据库或放宽校验强行接入。兼容项目再列服务、构建/验证/迁移命令、端口、源码路径及环境差异。
2. **先复用再适配。** 按[生成前检查](bootstrap.md#generator-preflight)确认本地依赖与输入，再运行包内生成器；已有项目则核对下表对应工件、版本和验收记录。同一阶段已有合用实现就不重写。未具备生成条件时保留待办，不手写将被生成器接管的候选事件、Tag UI 或归属锁。目标、路径和允许镜像仓库仍由项目批准的固定配置约束，不能变成任意流水线输入。
...
首次适配探针和业务验收时，从本项目的路由、全局响应封装、鉴权及代理配置确认请求路径和成功响应。复用核验方法，按项目分别判断响应；不要沿用其他项目的 URL 或用宽松兼容掩盖格式差异。首个云发布前，用实际生产构建或待发布镜像做最小 HTTP 检查，确认声明的健康、身份和页面入口存在；开发服务器上的浏览器测试不能替代这一步。镜像检查复用本次构建产物，不为验收另建一套发布镜像。

身份探针的 `api_envelope` 不是“这是 API”的标记：`false` 接受原始 `cnb-release-identity/v1` 对象；`true` 仅接受 `{"code":0,"message":"ok","data":{"release":<身份对象>}}`。新接口优先直接返回原始身份；已有业务响应封装不匹配时，只适配专用身份端点，并将真实 HTTP 响应交给生成包的校验器验证，不能只比对内部的 SHA/build 字段。
