...
5. **沿用标准发布。** 新项目构建、固定 TAT、候选及生产审批不变。验证新项目业务/恢复，使用 `scripts/verify-coexistence.py --spec <coexistence.json>` 对照已审基线；先离线预览，再加 `--apply` 只读采集 SSH/HTTPS 并保存本机证据。spec 声明本项目服务、受保护容器/文件、身份与访问探针，不在脚本中写死项目名或检查数量。再更新一次新项目，检查共存。普通发布始终不写 Caddy。

共存验证覆盖声明资源在观测时点的状态及基线差异，不证明持续负载或业务正确性。生产发布结果另由 `release-session.mjs verify` 回查；完成适用的业务、页面和恢复检查后，通过 `reconcile-project-state.py` 更新现有本地状态。完整输入与范围见[发布后收尾](post-release-closeout.md)。
