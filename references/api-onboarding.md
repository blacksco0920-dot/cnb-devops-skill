...
创建新仓库时，AI 按实际表单指引“资源范围 → 指定组织 → 已选父组织”（如 `team`），区分既有仓库 `team/app`；“仅应用于 team/app”不能覆盖同组织下其他新仓库。创建前核对资源选择及上表权限的读/写选项；“常见场景”卡片不替代逐项核对。缺权先核对当前令牌的实际选项，不要求重新登录。保存成功或公开组织 GET 成功不证明所需权限；API 成功也不能反推令牌只限某个组织或具有特定有效期。实际范围超出已有授权时按该范围交接。凭据仅用于本机初始化，不放进流水线；AI 私下记录范围、到期时间及其证据，未核实的信息保留 unknown。
...
仅凭生成文件或 API 成功不能报告流水线完成。当前进度分别记录：已发现/创建的资源及私密回执位置、Secret 的用户保存反馈、允许引用及范围外拒绝的核验结果、目标就绪状态、凭据到期时间、下一步；用户动作次数与实际等待时间分开记录。之后只继续失败或未完成阶段。
