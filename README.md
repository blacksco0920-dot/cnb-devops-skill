...
ecat 已有测试发布、候选 Tag 和生产页面的实操记录；其新生产执行入口仍待接通，不能把页面存在当成生产已验收。AI 的逐步执行说明、复用文件及证据范围见[标准工作流](references/standard-workflow.md)。
...
本包使用 Agent Skills Markdown 格式，没有运行时客户端依赖。AI 工具的最低能力是读取指引、读写项目文件、执行命令及访问 Git/HTTP API；实际脚本依赖按项目安装。不要求 Codex、Computer Use 或浏览器插件。必须本人完成或缺少可用 API 的控制台步骤，由 AI 准备好材料后交给你操作；浏览器自动化只作为可选辅助。
